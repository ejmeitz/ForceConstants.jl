...
# function three_body_second_nodes(pot::StillingerWeberSilicon, D)
...
#     H2_symbolic_ij = Matrix{FastDifferentiation.Node}(undef, D, D)
#     for a in range(1,D)
#         for b in range(1,D)
#             H2_symbolic_ij[a,b] = derivative([pot3_symbolic], r_j_vars[b], r_i_vars[a])[1]
#         end
#     end

#     H2_symbolic_ik = Matrix{FastDifferentiation.Node}(undef, D, D)
#     for a in range(1,D)
#         for b in range(1,D)
#             H2_symbolic_ik[a,b] = derivative([pot3_symbolic], r_k_vars[b], r_i_vars[a])[1]
#         end
#     end

#     H2_symbolic_jk = Matrix{FastDifferentiation.Node}(undef, D, D)
#     for a in range(1,D)
#         for b in range(1,D)
#             H2_symbolic_jk[a,b] = derivative([pot3_symbolic], r_k_vars[b], r_j_vars[a])[1]
#         end
#     end

#     r_vars = [r_i_vars; r_j_vars; r_k_vars]

#     return Dict("pair" => (H2_symbolic, r_ij_vars),
#                 "ij" => (H2_symbolic_ij, r_vars),
#                "ik" => (H2_symbolic_ik, r_vars),
#                "jk" => (H2_symbolic_jk, r_vars))
...
#     deriv_nodes = three_body_second_nodes(pot, D)
...
# function save_SW_second_derivs(pot::StillingerWeberSilicon, D, base_path::String)
#     deriv_nodes = three_body_second_nodes(pot, D)

#     for (key, dn) in deriv_nodes
#         filename = "H2_exec_$(key).jl"
